...
Color_Off='\033[0m'  
Cyan='\033[0;36m'  
echo -n "Enter the kernel: "
read kernel
echo -n "Enter the graph filename (in .sg format): "
read pow
echo -n "Enter the number of iterations: "
read iter
echo -e "${Green}Running Valgrind. Please wait...${Color_Off}"
echo ""
valgrind --tool=cachegrind --log-file=output.txt ./$kernel -f ./benchmark/graphs/$pow -n iter > output1.txt
echo ""
...
sudo perf stat ./$kernel -f ./benchmark/graphs/$pow -n iter > output.txt
